...
This table shows the number of unique publications and PRS by PRS type (development vs. evaluation). The numbers count results from PRIMED publications only. A single publication can include both development and evaluation of a PRS.

XXX maybe try having a "total" and "subset" rows
...
This table shows the number of unique publications, the number of unique PRS developed, and the number of unique PRS evaluated for each trait. The numbers count results from PRIMED publications only.

XXX Add total number of unique sample sets evaluated, total number of evaluations for each trait
